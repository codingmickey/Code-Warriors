...
const WalletView = () => {
  const { tz } = useParams();
  const { walletId, setWalletId, objkts, isLoading, setIsLoading, error } =
    useWallet();
  const [walletsWithAudio, setWalletsWithAudio] = useState([]);
  useTitle(`Audious ${walletId ? `| ${walletId}` : ''}`);
  useEffect(() => {
  ...
  useEffect(() => {
    (async () => {
      ...
      const uniqueWalletsSet = new Set(
        data?.hic_et_nunc_token?.map((o) => o.creator_id),
      );
      const nextCreatorMetadata = (
        await Promise.allSettled(
          [...uniqueWalletsSet].map((id) => getUserMetadataByWalletId(id)),
        )
      )
